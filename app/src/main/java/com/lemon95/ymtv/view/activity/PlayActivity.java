      ...
      //  mVideoView.setMediaController(videoJjMediaContoller);
        ...
        mVideoView.setOnJjOpenSuccess(new OnJjOpenSuccessListener() {
            ...
            public void onJjOpenSuccess() {
                ...
                if (lastWatchTime > 0) {
                    mVideoView.seekTo(lastWatchTime);
                    showMsg("定位到上次播放位置");
                }
                if (isPro) {
                    showLL();
        ...
        mVideoView.setOnJjCompletionListener(new OnJjCompletionListener() {
            ...
            public void onJjCompletion() {
                if (serialEpisodes != null && serialEpisodes.size() > index && (AppConstant.SERIALS.equals(videoType)|| AppConstant.ZONGYI.equals(videoType) || AppConstant.DONGMAN.equals(videoType))) {
                    showMsg("自动进入下一集");
...
/***
       ...
       // mVideoView.setVideoJjAppKey("N1fd2rFDZ");
        mVideoView.setVideoJjAppKey("Hk3Ptw3yl");
        mVideoView.setVideoJjPageName("com.xiangjia");
...
/**
    ...
    public void startPlay(String url) {
        ...
        if (AppConstant.MOVICE.equals(videoType)) {
            ...
            moviesLog = dataBaseDao.findMoviesLogByMovieId(videoId, videoType,index);
        }
        if (moviesLog != null && StringUtils.isNotBlank(moviesLog.getWatchTime())) {
            lastWatchTime = Integer.parseInt(moviesLog.getWatchTime());
        }
